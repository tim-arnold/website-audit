...
The goal is to identify and prioritize security issues on the existing site and produce an actionable remediation plan. The site is not being rebuilt — findings should be fixable on the current codebase and hosting configuration.
...
- `reports/security-audit-report.md` — dev-team report with prioritized remediation checklist
...
- **Risk**: the attack vector or data exposure it enables
- **Fix**: what needs to change (specific header, config file, package upgrade, code change)
- **Effort**: time estimate

Organize by tier:

**Critical (fix immediately)** — exploitable now without attacker privileges: exposed credentials, no HTTPS, known-exploited CVEs, publicly accessible admin interfaces
**High (fix soon)** — low-effort exploits or significant data exposure: missing security headers, cookies without `Secure`/`HttpOnly`, high-severity CVEs, information disclosure
**Medium (planned work)** — requires chaining or specific conditions: CSP gaps, missing CSRF tokens on low-risk forms, moderate CVEs, missing SRI on third-party scripts
**Low (backlog)** — best-practice gaps with minimal direct risk: header hardening, cookie `SameSite` refinements, minor dependency updates
...
- Cross-reference technology audit findings where dependency versions or hosting configuration are mentioned
